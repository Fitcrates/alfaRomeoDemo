...
function TrackModel({ onCollidersLoaded, onTerrainLoaded }) {
  const config = TRACK_CONFIGS.drift;
  const isKarting = false;
  ...
  useEffect(() => {
    if (scene && groupRef.current) {
      ...
      console.log(`[TrackModel:drift] Colliders:`, colliders.length, '| Terrain meshes:', terrainMeshes.length);
...
export default function DriftTrackScene({
  carColor,
  headlightsOn,
  driveDirectionRef,
  driveMode = 'dynamic',
  ...
  const isKarting = false;
  const config = TRACK_CONFIGS.drift;
  const terrainTuning = TRACK_TUNING.drift;
